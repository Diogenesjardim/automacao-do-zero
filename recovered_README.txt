# ­ƒÜÇ Automa├º├úo do Zero

Projeto para iniciantes aprenderem automa├º├úo de testes.

## ­ƒôÜ Etapas

1. Setup ÔåÆ /01-setup  
2. Git e GitHub ÔåÆ /02-git-github  
3. Java API ÔåÆ /03-java-api  
4. Java Web ÔåÆ /04-java-web  
5. Cypress API ÔåÆ /05-cypress-api  
6. Cypress Web ÔåÆ /06-cypress-web

## ­ƒøá´©Å Configura├º├úo do Ambiente
...
3. **Configurar vari├íveis de ambiente**:
...
## ­ƒº¬ Executando os Testes

### Java Web (/04-java-web)

1. Navegue para a pasta: `cd 04-java-web`
2. Execute: `mvn test`
   - O WebDriverManager baixar├í automaticamente o ChromeDriver.
   - O teste abrir├í o Chrome e acessar├í https://example.com.

### Poss├¡veis Erros e Corre├º├Áes
...
- **Erro: "TestEngine with ID 'junit-jupiter' failed to discover tests"**: Certifique-se de que a classe de teste est├í em um pacote (ex: `com.estudo`).
- **Erro: "class file version"**: Use JDK 17+ e configure o pom.xml com `<maven.compiler.source>17</maven.compiler.source>`.
- **Erro: ChromeDriver n├úo encontrado**: O WebDriverManager cuida disso; certifique-se de que o Chrome est├í instalado.
...
Para mais detalhes, consulte o `pom.xml` em cada m├│dulo.  
